...
def main() -> None:
    # 1. Olvassa be az UTF-8 kódolású  legmagasabb.txt állományban lévő adatokat
    ...
    # 2. Határozza meg és írja ki a képernyőre a minta szerint,
    # hogy hány épület található az állományban!
    print(f'2. feladat: Épületek száma: {len(épületek)} db')

    # 3. Határozza meg és írja ki a képernyőre a minta szerint az állományba
    # található épületek emeleteinek az összegét!
    emeletek_összege: int = 0
    for e in épületek:
        emeletek_összege += e.emelet
    print(f'3. feladat: Emeletek összege: {emeletek_összege}')

    # 4. Határozza meg és írja ki a képernyőre a minta szerint, a legmagasabb épület
    #  adatait! Feltételezheti, hogy nem alakult ki holtverseny!
    print('4. feladat: A legmagasabb épület adatai')
    ...
    for e in épületek:
        if e.ország == 'Olaszország':
            ...
            break
    print(f'5. feladat: {"Van" if van_olasz_épület else "Nincs"} olasz épület az adatok között!')
